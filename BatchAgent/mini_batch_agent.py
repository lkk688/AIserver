...
"""
mini_batch_agent.py (modified from mini_code_agent_v2.py)
...
import asyncio # [NEW] Required for batch parallelism
import urllib.request # [NEW] For lightweight web search
# [MODIFIED] Use AsyncOpenAI for vLLM batching
...
class AgentConfig:
    ...
    rl_mode: bool = False
    max_retries: int = 4
    batch_size: int = 1       # [NEW] Number of parallel requests to send to vLLM
    search_api_key: str = ""  # [NEW] For Web Search
...
ANTHROPIC_TOOLS = [
    ...
    # [NEW] Added web_search to schema
    {
        "name": "web_search",
        "description": "Search the internet for real-time information, documentation, or technical solutions.",
        "input_schema": {
            "type": "object",
            "properties": {
                "query": {"type": "string", "description": "The exact search query to send to Google."}
            },
            "required": ["query"]
        }
    }
...
class PromptRegistry:
    ...
    SYSTEM = (
        ...
        "`<tool_call><search_code>query</search_code></tool_call>`\n"
    )
