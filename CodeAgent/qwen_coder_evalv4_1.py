...
New Features:
- Evaluates models from Hugging Face (HF), local folders, local vLLM, or remote vLLM with passkey.
- Simple model specification via command line with a model source selector.
...
python CodeAgent/qwen_coder_evalv4_1.py \
  --models "unsloth/Qwen2.5-Coder-14B-Instruct" "./qwen_coder_lora" "openai:gpt-4o" \
  --run_all --make_figures \
  --out_dir ./eval_results

# Evaluate a remote vLLM model with passkey
export VLLM_API_KEY="myhpcvllmqwen"
python CodeAgent/qwen_coder_evalv4_1.py \
  --model_source remote_vllm \
  --remote_vllm_url "https://w0wqtv67-8000.usw3.devtunnels.ms/v1" \
  --models "Qwen/Qwen3-Coder-Next-FP8" \
  --run_all --out_dir ./eval_results_remote
...
def install_required_packages() -> None:
    ...
    run_cmd(base + ["--upgrade", "setuptools", "wheel", "pip"])
    run_cmd(base + ["openai"])
    run_cmd(base + ["evalplus"])
    run_cmd(base + ["lm-eval"])
...
def resolve_openai_api_key(spec: ModelSpec) -> Optional[str]:
...
def parse_model_spec(model_str: str, args: argparse.Namespace) -> ModelSpec:
    ...
    if args.model_source != "auto":
        if args.model_source == "remote_vllm":
            if not args.remote_vllm_url:
                raise ValueError("remote_vllm requires --remote_vllm_url")
            return ModelSpec(
                name=model_str,
                path=model_str,
                type="api",
                api_base=args.remote_vllm_url,
                api_key_env=args.remote_vllm_api_key_env,
                is_local_api=False,
                api_timeout=args.api_timeout,
                api_retries=args.api_retries,
                api_retry_backoff=args.api_retry_backoff,
            )
        if args.model_source == "local_vllm":
            if not args.api_base:
                raise ValueError("local_vllm requires --api_base")
            return ModelSpec(
                name=model_str,
                path=model_str,
                type="api",
        ...
        if args.model_source == "api":
            return ModelSpec(
                ...
                api_base=args.api_base,
                api_key_env=args.api_key_env,
                is_local_api=bool(args.api_base and ("127.0.0.1" in args.api_base or "localhost" in args.api_base)),
                api_timeout=args.api_timeout,
                api_retries=args.api_retries,
                api_retry_backoff=args.api_retry_backoff,
            )
        if args.model_source == "local_hf":
            if not os.path.isdir(model_str):
                raise ValueError("local_hf requires a local directory path")
...
def create_api_generator(model_spec: ModelSpec, max_new_tokens: int) -> Callable[[str], str]:
    api_key = resolve_openai_api_key(model_spec)
    if not api_key and not model_spec.is_local_api:
        raise ValueError(f"API key not found. Please set the {model_spec.api_key_env} environment variable.")
    api_base = normalize_api_base(model_spec.api_base)
    if not api_base:
        raise ValueError("API base URL not set. Please provide --api_base or --remote_vllm_url.")
    ...
    def generator(prompt_text: str) -> str:
        ...
        for attempt in range(model_spec.api_retries + 1):
            try:
                with urllib.request.urlopen(req, timeout=model_spec.api_timeout) as resp:
                    raw = resp.read().decode("utf-8")
                if not raw:
                    raise RuntimeError("API response body is empty")
                data = json.loads(raw)
                if "error" in data:
                ...
                last_error = e
                if e.code in retry_codes and attempt < model_spec.api_retries:
                    time.sleep(model_spec.api_retry_backoff * (2 ** attempt))
                    continue
                err_body = e.read().decode("utf-8") if e.fp else ""
                raise RuntimeError(f"API request failed: {e.code} {e.reason} {err_body}")
            except (urllib.error.URLError, json.JSONDecodeError, RuntimeError) as e:
                last_error = e
                if attempt < model_spec.api_retries:
                    time.sleep(model_spec.api_retry_backoff * (2 ** attempt))
                    continue
...
def run_evalplus(spec: ModelSpec, dataset: str, parallel: int, out_dir: str) -> Dict:
    ...
    if spec.type == "api":
        ...
        api_base = normalize_api_base(spec.api_base)
        if api_base: cmd.extend(["--base-url", api_base])
    else:
    ...
    env = os.environ.copy()
    if spec.type == "api":
        api_key = resolve_openai_api_key(spec)
...
def run_lm_eval(spec: ModelSpec, tasks: str, bs: int, dev: str, out_dir: str) -> Dict:
    ...
    if spec.type == "api":
        if spec.is_local_api:
            ...
            api_base = normalize_api_base(spec.api_base)
            if api_base:
                model_args_parts.append(f"base_url={api_base}")
            cmd.extend(["--model", "openai-chat-completions", "--model_args", ",".join(model_args_parts)])
    else:
...
def main():
    ...
    # Core
    parser.add_argument("--models", nargs='+', default=["unsloth/Qwen2.5-Coder-14B-Instruct", "qwen_coder_lora_sft_v3_2"], help="Models to evaluate. Use --model_source to control how models are interpreted.")
    parser.add_argument("--model_source", type=str, default="auto", choices=["auto", "remote_vllm", "local_hf", "local_vllm", "api"], help="Select model source: auto (infer), remote_vllm, local_hf, local_vllm, api.")
    ...
    parser.add_argument("--api_timeout", type=int, default=120, help="Timeout in seconds for API requests.")
    parser.add_argument("--api_retries", type=int, default=2, help="Number of retries for API requests.")
    parser.add_argument("--api_retry_backoff", type=float, default=1.5, help="Backoff multiplier for API retries.")
    parser.add_argument("--remote_vllm_url", type=str, default=None, help="Base URL for remote vLLM OpenAI-compatible API.")
    parser.add_argument("--remote_vllm_api_key_env", type=str, default="VLLM_API_KEY", help="Environment variable for remote vLLM API key.")
    parser.add_argument("--remote_vllm_api_key", type=str, default=None, help="Remote vLLM API key to set for this run only.")
    parser.add_argument("--install_deps", action="store_true", help="Install evalplus and lm-eval requirements.")
    ...
    if args.remote_vllm_api_key:
        os.environ[args.remote_vllm_api_key_env] = args.remote_vllm_api_key
